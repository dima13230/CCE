...
public static class NeitriExtensions
{
    ...
    public static T FindChildOfType<T>(Node parent)
    {
        ...
        {
            // Found a child of the desired type
            return typedNode;
        }

        // Iterate through each child and recursively search
        foreach (Node child in parent.GetChildren())
        {
            T foundChild = FindChildOfType<T>(child);
            if (foundChild != null)
            {
                // Found the child in the hierarchy
                return foundChild;
            }
        }

        // Child of the specified type not found in the hierarchy
        return default;
    }

	public static List<T> FindChildrenOfType<T>(Node parent)
    {
		List<T> children = new List<T>();
        if (parent is T typedNode)
        {
            // Found a child of the desired type
			children.Add(typedNode);
        }

        // Iterate through each child and recursively search
        foreach (Node child in parent.GetChildren())
        {
            List<T> foundChildren = FindChildrenOfType<T>(child);
